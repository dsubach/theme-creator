...
import { saveEditorToTheme, useUpdateEditorState } from 'src/state/actions';
...
async function validateInput(editorRef: EditorRefType) {
  ...
  if (model == null) return [null, null, null];
...
export const useSave = (editorRef: EditorRefType) => {
  ...
  const handleSave = useCallback(async () => {
    ...
    const [semanticDiagnostics, syntacticDiagnostics, emittedOutput] = await validateInput(
      editorRef,
    );

    // if there are semantic errors, prevent saving, else save to redux store
    let errors: monaco.languages.typescript.Diagnostic[] = [];
    if (syntacticDiagnostics != null) {
      errors = [...errors, ...syntacticDiagnostics];
    }

    if (semanticDiagnostics != null) {
      errors = [...errors, ...semanticDiagnostics];
    }
    if (errors.length > 0) {
      // handle errors
 ...
 * @param onSave
 */
export const useSaveKey = (editorRef: EditorRefType, onSave: () => void) => {
  useEffect(() => {
