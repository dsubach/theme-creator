  ...
  useEffect(() => {
    ...
    const preventModifyReadOnlyLines = (event: monaco.IKeyboardEvent) => {
      ...
      const lastEditableLine =
        ((editorRef.current?.getModel()?.getLineCount() as number) ?? 0) - readOnlyLines.bottom;

      if (selection == null || lastEditableLine < 0) return null;
      ...
      if (
        ...
        const eventIsPaste = event.ctrlKey && event.keyCode === monaco.KeyCode.KeyV;
        const eventIsCut = event.ctrlKey && event.keyCode === monaco.KeyCode.KeyX;
    ...
    const preventDeleteToReadOnlyLines = (event: monaco.IKeyboardEvent) => {
      if (event.keyCode === monaco.KeyCode.Delete) {
        const selection = editorRef.current?.getSelection();
        const lastEditableLine =
          (editorRef.current?.getModel()?.getLineCount() || 0) - readOnlyLines.bottom;
